...
function accountNumberFromLine (line) {
  return parseLine(line)
    .map(vector => (characterMap[vector] === undefined)
         ? '?'
         : characterMap[vector])
...
// validateAccount :: (String) -> String
//
// Accepts accountNumber generated by =accountNumberFromLine=.
// Returns a string according to User Story #3
function validateAccount (accountString) {
  if (/\?/.test(accountString)) {
    return `${accountString} ILL`;
  } else if (!checksum(accountString)) {
    return `${accountString} ERR`;
  } else {
    return accountString;
  }
}
